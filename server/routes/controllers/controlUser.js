...
module.exports = {
    ...
    getUserByLogin : async(request, reply) => {
        try {
            if (!request.params || !request.params.login)
                throw new CustomError(1023);
            if (!request.user) request.user = {role: 'user'};
            const userModel = new User(request.db.sequelize.models.Users);
            const users = await userModel.getByLogin(request.user.role, request.params.login);
            reply.status(200).send(users);
        } catch (error) {
            errorReplier(error, reply);
        }
    ...
    editUserAvatar : async(request, reply) => {
        try {
            ...
            await user.edit({profilePic: avatarName}, previousPawn.id, false, request.user.login);
